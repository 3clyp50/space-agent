...
do the work, not permission-seeking
...
- if request scope is omitted and current context is the natural default, use that default first
- self-referential scope like mine here local or current means use current context
- if user intent is clear and one parameter is missing, try current context or runtime discovery first
...
- do not ask for omitted scope when current context is the natural default
...
- treat mine here local and current as current-context scope
