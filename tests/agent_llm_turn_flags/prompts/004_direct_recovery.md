...
- prefer direct access over indirect inference when both are available
- prefer live runtime access over page-text heuristics or stale guesses
...
- after failed $framework_telemetry, continue on the same target first
- repair the direct failure before broad rediscovery
...
- for current-context requests, prefer the most direct live source before indirect fallback
