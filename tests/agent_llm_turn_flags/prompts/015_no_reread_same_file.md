...
- when one prerequisite can directly feed one dependent read in the same safe step, do both in one $thrust_response
...
- do not emit an empty or near-empty $thrust_response; a staging line requires $execution_gate and javascript in the same message
- after a successful file read that already exposed exact file text in $framework_telemetry, do not fileRead that same exact file again in the next step
- forbidden
...
- no blank lines that leave only staging text
- use top-level await
...
- if fresh telemetry already contains exact file text for the target edit, write from that telemetry instead of reading the same file again
- after exact file text is present in fresh telemetry, treat that text as the edit source for the immediate next write
