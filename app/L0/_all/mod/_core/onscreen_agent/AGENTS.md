...
- the settings modal keeps a provider switch with three tabs named `API`, `ChatGPT`, and `Local`; API settings show endpoint, model, and API key fields, `ChatGPT` settings own the OpenAI Codex OAuth device-code login plus a model dropdown sourced only from `discoverCodexModels(...)` via the signed-in account catalog, and local settings mount the shared `_core/huggingface/config-sidebar.html` component in `onscreen` mode
